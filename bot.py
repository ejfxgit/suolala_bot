...
async def generate_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    last_used = GENERATE_COOLDOWNS.get(user_id, 0)
    if now - last_used < GENERATE_COOLDOWN_SECONDS:
        wait = int(GENERATE_COOLDOWN_SECONDS - (now - last_used))
        msg = await update.message.reply_text(f"⏳ Wait {wait}s before generating again.")
   ...
   try:
        ...
        if not choices:
            print(f"[GENERATE] Empty choices: {data}")
            await thinking_msg.delete()
            await update.message.reply_text("❌ Image generation failed.")
            return
        ...
        if isinstance(content, list):
            for part in content:
                if isinstance(part, dict):
                    if part.get("type") == "image_url":
                        ...
                        raw = url_obj.get("url", "") if isinstance(url_obj, dict) else url_obj
            ...
            if content.startswith("http"):
                ...
                # Try treating the raw string as base64
                try:
        ...
        if image_data:
            ...
            buf.name = "output.png"
            await update.message.reply_photo(photo=buf, caption=f"🎨 {user_prompt}")
        elif image_url:
            await update.message.reply_photo(photo=image_url, caption=f"🎨 {user_prompt}")
        else:
            print(f"[GENERATE] Could not extract image from response: {data}")
            await update.message.reply_text("❌ Image generation failed.")
